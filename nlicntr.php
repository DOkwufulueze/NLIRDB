...
<?php
	if($_POST){
			...
			$poss=0; $termn=array(); $request_wordn=array();
			for($req=0;$req<count($holder);$req++){
				...
				if($num2!=0){//To check for the occurence of Non-Entity-Referencing words(NER)
					...
					$term=nlirdb::removeDuplicate($term,$termn);
					$request_word=nlirdb::removeDuplicate($request_word,$request_wordn);
				}
			}
			$rr2=0;
			 ...
			 *************************************************/
			 if(empty($genValid)&&empty($speValid)){
			 	if(!empty($request_word)){
							...
							$reqq=nlirdb::concatWithSymbol($request_word, ","); //In case I want to see the SQL-Equivalent Statement
							if(nlirdb::isFind("COUNTRY",$request_word)){
								$qq = mysql_query("SELECT * FROM countries ") or die(mysql_error());
								...
								if($nu!=0){
									...
									while($roww=mysql_fetch_array($qq)){
										if(count($request_word)>1){
											...
											if($roww[$request_word[$here]]!=""){
												if($term[$here]=="Continent"){
													if(!nlirdb::isFind($roww['LOCATE'].$roww[$request_word[$here]], $used)){
														$used[]=$roww['LOCATE'].$roww[$request_word[$here]];
														echo strtoupper($term[$here])." : ".$roww['LOCATE']." ".$roww[$request_word[$here]]."<br/><br/>";
													}
												}
												else{
													echo strtoupper($term[$here])." : ".$roww[$request_word[$here]]."<br/>";
			...
			elseif(empty($genValid)&&!empty($speValid)){
				$count=0; $counts=0; $cou=array(); $cou2=array(); $k=0;$name=array(); $cont=array();
				...
				for($i=0;$i<count($speValid);$i++){
					$field=$speDescr[$i]; $value=$speValid[$i]; 
						if(!empty($request_word)){
							...
							if(nlirdb::isFind("COUNTRY",$request_word)){
								if(nlirdb::isFind($nth,$speValid)&&!nlirdb::isFind($sth,$speValid)){
									$loc=$nth;
									$qq = mysql_query("SELECT * FROM countries WHERE ".$field."='$value' AND LOCATE='$loc'") or die(mysql_error());
								}
								else if(!nlirdb::isFind($nth,$speValid)&&nlirdb::isFind($sth,$speValid)){
									$loc=$sth;
									$qq = mysql_query("SELECT * FROM countries WHERE ".$field."='$value' AND LOCATE='$loc'") or die(mysql_error());
								}
								else if(nlirdb::isFind($nth,$speValid)&&nlirdb::isFind($sth,$speValid)){
									$loc1=$nth ;
									$loc2=$sth;
									$qq = mysql_query("SELECT * FROM countries WHERE ".$field."='$value' AND (LOCATE='$loc1' OR LOCATE='$loc2')") or die(mysql_error());
								}
								else{
									$loc="";
						...
						else{//NER words are not in the user's query
							//echo"<tr><td>";
							if(nlirdb::isFind($nth,$speValid)&&!nlirdb::isFind($sth,$speValid)){
								$loc=$nth;
								$qq = mysql_query("SELECT * FROM countries WHERE ".$field."='$value' AND LOCATE='$loc'") or die(mysql_error());
							...
							if($nu!=0){
								while($roww=mysql_fetch_array($qq)){
									...
									if(!nlirdb::isDuplicate($cou)){ 
										echo"<tr><td>";
										echo"Country : ".$roww['COUNTRY']."<br/>".
											"Continent : ".$roww['LOCATE']." ".$roww['CONTINENT']."<br/>".
